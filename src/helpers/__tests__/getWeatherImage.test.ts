...
// Convert the weatherData object into an array of test cases
const testCases = Object.entries(weatherData).map(([weatherCode, { day }]) => [
  weatherCode,
  day.image,
]);
...
test.each(testCases)(
  ...
  (weatherCode, expectedImage) => {
    // Access the image property from the result of getWeatherImage
    const result = getWeatherImage(weatherCode, true);
    expect(result.image).toEqual(expectedImage);  // Compare the image property
  },
);